...
- Existing Lythaus deterministic V1 forensics remain experimental measurements. Compressed input bytes remain available for EF1 container/provenance analysis, while the runner decodes JPEG, PNG, and WebP pixels through the Sharp raw-buffer path before EF2/EF4 execution. If decoding fails, EF2 and pixel-domain EF4 are explicitly unavailable; EF3 and EF5 are unavailable until implemented.
...
Live provider use requires `--allow-network` and an explicit `--max-samples`. Local execution defaults to mocks/network-disabled. The CLI uses the REST adapters for live Observer/Judge modes only when the two Cloudflare credentials are supplied. The runner enforces separate moderation, direct Observer, reasoned Observer, task-specific Observer, Judge, total-call, one-recheck, and two-Judge-pass caps. Normal direct mode is one Observer request per image; eligible uncertain direct output may add exactly one owned reasoned query before the first Judge. Judge-directed rechecks use the same single reasoned-pass budget; if the Observer already consumed it, the Judge cannot trigger another pass. A/B is one direct plus one reasoned query; `caption`, `detect`, and `point` each represent one provider request when selected. There is no ROI follow-up or recursive Observer loop.

Packet aggregate quality is `AVAILABLE` only when the configured families are complete, `PARTIAL` when known families such as EF3/EF5 are unavailable, `FAILED` when an executed component fails, and `CONTRADICTORY` when material observations or measurements conflict. Missing families remain explicit in every packet.
