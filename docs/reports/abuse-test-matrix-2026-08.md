...
| Duplicate activity delivery | Unique `(user_id, source_event_id, event_type)` provides at-most-one persisted activity effect after migration. | Migration, `recordUserActivity`, and the catalogue-keyed public-API inventory use the same source identity; admin/jobs have separate source invariants and sequential replay returns the original. | **Partial** — extend the declared inventory across admin/jobs/workflows, then run concurrent Worker/outbox retries and prove at-most-one durable effect plus reliable result retrieval/notification reconciliation. |
...
- `npm run test:critical-coverage` — **passed in the current worktree** for 20 modules across 12 domains, including extracted invoked runtime seams; this is not whole-system or deployment coverage.

The first two are policy-level checks; the source-derived mutation inventory remains static verification while the coverage gate exercises extracted runtime seams. They are useful regression controls, but do not substitute for full Worker/Hyperdrive transactions, migrated schema, DSR, queue, deployment, and authenticated Flutter contract tests.
