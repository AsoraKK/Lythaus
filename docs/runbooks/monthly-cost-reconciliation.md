...
| PlanetScale hosted `development` branch | Not found in the 2026-08-22 read-only inventory |
| Known forecast floor from prior estimates | approximately US$10/month |
...
Do not recreate a PlanetScale `development` branch solely to satisfy older
documentation. Use local PostgreSQL 17 for compatibility validation unless an
approved synthetic-data, recovery, dependency, and cost assessment establishes
a current hosted-branch need. Any provider branch change remains separately
approval-gated.
