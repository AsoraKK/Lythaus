# Lythaus PlanetScale baseline

This directory is the authoritative migration source for the Cloudflare-native
Lythaus runtime. Retired provider files are not part of this baseline.

Execution rules:

- Apply migrations through the direct PlanetScale administrative connection.
- Never run migrations through Hyperdrive.
- Use a disposable local PostgreSQL 17 container for complete compatibility
  tests. Never create an ephemeral PlanetScale branch from CI.
- A read-only inventory on 2026-08-22 found only the production `main` branch;
  the previously documented `development` branch was not found. Do not
  recreate it or delete any provider resource without a separate approved
  dependency, synthetic-data, recovery, and cost assessment.
- Do not execute writes or DDL against `main` without explicit human approval.
- Reinstall required extensions after every backup restore because restored
  branches do not automatically restore extensions. PostGIS is optional and
  deferred; it must not be added to a launch gate.

PlanetScale's managed role API returns generated SQL identifiers such as
`pscale_api_<role-id>.<branch-id>`. The display labels (`lythaus_runtime`,
`lythaus_admin`, `lythaus_jobs`, `lythaus_privacy`, and `lythaus_migrations`)
are repository-level policy names, not SQL role names in the web-console
session. CI and direct migration runners must resolve and validate the
generated `pscale_api_*` prefix before substituting it into `grants/roles.sql`.
Never paste credentials or role reset output into source control or evidence.

The existing Frankfurt PostgreSQL 17 `main` branch is authoritative. Do not
upgrade its engine, region, compute, replicas, or billing plan. New UUIDv7
values are generated by Workers and migration scripts and supplied explicitly
to native `uuid` columns.

The current provider snapshot is recorded in
`docs/evidence/production-cutover/2026-08-22-read-only-provider-inventory.json`.
Older references to a PlanetScale `development` branch are historical context,
not live provider evidence.
