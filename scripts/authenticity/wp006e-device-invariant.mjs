...
async function transformationStress(options, cameraRows, state, model, threshold, finalRegistryHash, selectionPolicyHash, configurationHash, benchmarkFingerprint) {
  ...
  for (const transform of transforms) {
    ...
    for (const row of selected.slice(0, 8)) {
      const bytes = await resolveRuntimeBytes(row, options, state);
      const decoded = await decodeRaw(sharp, bytes);
      let input = decoded;
...
function renderReport({ plan, stageA, developmentResults, featureSelectionResults, shortcutAudit, finalRegistry, finalRegistryHash, runManifest, validation, holdout, transformation, decision, classification, next }) {
...
function cameraPartitionsCount(_runManifest, _role) {
  return 48;
}
